...
class Simulation:
    ...
    def run(self, steps, debug=simulation_debug):
        ...
        for i in range(steps):
            ...
            self.measurement_module.save_step(self.get_sim_time(), self.cars)
        self.previous = time.time()
        if self.finished():
            self.measurement_module.save_measurements(self.map_name)
            pygame.quit()
            sys.exit()
    ...
    def finish(self):
        self.measurement_module.save_measurements(simulation_name=self.map_name)
